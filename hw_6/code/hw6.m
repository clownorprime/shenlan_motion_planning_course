...
for k = 1:n_seg
    for t = 0:0.01:1
        ...
        for i = 0:n_order
            ...
            x_coord = x_coord + poly_coef_x((k - 1) * (n_order + 1) + i + 1) * basis_p;
            y_coord = y_coord + poly_coef_y((k - 1) * (n_order + 1) + i + 1) * basis_p;
        end
        x_pos(:, end + 1) = x_coord;
        y_pos(:, end + 1) = y_coord;
    end
...
function poly_coef = MinimumSnapCorridorBezierSolver(axis, waypoints, corridor, ts, n_seg, n_order, v_max, a_max)
    ...
    for i = 1 : n_seg
        p_min = corridor(axis, i) - corridor(2 + axis, i);
        p_max = corridor(axis, i) + corridor(2 + axis, i);
        corridor_range(i, 1) = p_min;
        corridor_range(i, 2) = p_max;
    end
    corridor_range
    ...
    [poly_coef, FVAL, EXITFLAG] = quadprog(Q_0,f,Aieq, bieq, Aeq, beq);
end
