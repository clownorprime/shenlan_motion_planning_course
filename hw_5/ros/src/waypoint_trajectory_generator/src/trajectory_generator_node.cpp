...
void rcvWaypointsCallBack(const nav_msgs::Path & wp)
{   
    ROS_INFO_STREAM("get waypoints topic.");
...
void trajGeneration(Eigen::MatrixXd path)
{
    ROS_INFO_STREAM("path row: " << path.rows());
    std::cout << "path : \n";
    for (int i = 0; i < path.rows(); i++)
    {
        std::cout << "row " << i << ":";
        std::cout << path.row(i) << std::endl;;
    }
...
{ 
    ...
    */
    ...
    double rise_time = _Vel / _Acc;
    std::cout << "Vel_, vel_, rise_time: " << _Vel << " " << _Acc << " " << rise_time << std::endl;
    for (int i = 0; i < Path.rows() - 1; i++)
    {
        double x_dist = Path(i + 1, 0) - Path(i, 0);
        double y_dist = Path(i + 1, 1) - Path(i, 1);
        double z_dist = Path(i + 1, 2) - Path(i, 2);
        double distance = std::sqrt(std::pow(x_dist, 2) + std::pow(y_dist, 2) + std::pow(z_dist, 2));
        if (_Vel * rise_time > distance)
        {
            time[i] = std::sqrt(2 * distance / _Acc);
        }
        else
        {
            time[i] = 2 * rise_time + (distance - _Vel * rise_time) / _Vel;
        }
        std::cout << "path " << i << ", distance: " << distance << " time: " << time[i] << std::endl;
    }
    return time;
}
