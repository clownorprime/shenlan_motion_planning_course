...
{   
    ...
    ROS_INFO_STREAM("cloud point size: " << cloud.points.size());
    pcl::PointXYZ pt;
    ...
    if (solved)
    {
        ...
        for (size_t path_idx = 0; path_idx < path->getStateCount (); path_idx++)
        {
            ...
            */ 
            ...
            _RRTstar_preparatory->isObsFree(state->values[0], state->values[1], state->values[2]);
            ROS_INFO_STREAM("point: " << state->values[0] << " " << state->values[1] << " " << state->values[2]);
            ROS_INFO_STREAM("obs free ? " << _RRTstar_preparatory->isObsFree(state->values[0], state->values[1], state->values[2]));
        }
