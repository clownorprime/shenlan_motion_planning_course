...
void execCallback(const ros::TimerEvent &e) {
  ...
  case EXEC_TRAJ: {
    ...
    if (print_count_global++ % 5 == 0) 
    {
      std::cout << "time duration: " << time_duration << " t_cur: " << t_cur << " odom: " << odom_pt[0] << " " << odom_pt[1] << " " << odom_pt[2] << std::endl;
      std::cout << "target pt:" << target_pt[0] << " " << target_pt[1] << " " << target_pt[2] <<  " t_replan " << t_replan << std::endl;
      std::cout << "no_replan_thresh: " << no_replan_thresh << " replan thresh: " << replan_thresh << std::endl;
    ...
    if (t_cur > time_duration - 1e-2) {
      ...
      ROS_INFO_STREAM("t_cur is less than t_replan, t_cur: " << t_cur << " " << t_replan);
      return;
    } else {
      ROS_INFO_STREAM("ready to REPLAN TRAJ");
...
bool trajGeneration() {
   ...
   *
   * **/
  ROS_INFO_STREAM("target pt: " << target_pt[0] << " " << target_pt[1] << " " << target_pt[2]);
  ...
  if (grid_path.size() <= 3)
  {
    std::cout << "astar find path size less than 3, we need regenerate trajectory." << std::endl;
    return false;
  }
  grid_path = _astar_path_finder->pathSimplify(grid_path, _path_resolution);
  std::cout << "simplify path size: " << grid_path.size() << std::endl;
  for (int i = 0; i < grid_path.size(); i++)
  {
    auto point = grid_path[i];
    std::cout << "point " << i << " : " << point[0] << " " << point[1] << " " << point[2] << std::endl;
  ...
  if (grid_path.size() < 2)
  {
    std::cout << "simplify path size less than 2, we need regenerate trajectory." << std::endl;
    return false;
  }
...
VectorXd timeAllocation(MatrixXd Path) {
  ...
  //speed: _Vel acc: _Acc
  double sides_distance = _Vel * _Vel / _Acc;
  for (int i = 0; i < Path.rows() - 1; i++)
  {
    VectorXd first = Path.row(i);
    VectorXd second = Path.row(i + 1);
    double distance = std::sqrt(std::pow(first[0] - second[0], 2) + std::pow(first[1] - second[1], 2) + std::pow(first[2] - second[2], 2));
    if (distance < sides_distance)
    {
      time[i] = std::sqrt(2 * distance / _Acc);
    }
    else
    {
      time[i] = 2 * sides_distance / _Vel + (distance - sides_distance) / _Vel;
    }
  }
  return time;
}
...
int main(int argc, char **argv) {
  ...
  nh.param("path/resolution", _path_resolution, 0.2);
